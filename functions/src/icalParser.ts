...
// ─── iCal → ScrapedEvent ────────────────────────────────────────────────────
// Ported verbatim from the old server/_core/ical-parser.ts.
//
...
// schema enum is the source of truth — anything not on this list is `null`
...
export function parseIcal(
  ...
  for (const vevent of vevents) {
    ...
    // Defensive truncation: title is capped at 500 chars in the schema.
    ...
    // midnight and keep them. DTEND on a VALUE=DATE event is exclusive (the
    // day after the last day); we keep it as the endAt boundary and include
    // the event if it overlaps the window at all.
