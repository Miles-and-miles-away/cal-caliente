...
**Last Audited:** 2026-05-04 (after v1.5: iCal + venue dedup + race fix)
**Test Coverage:** 339 passing, 1 skipped (auth.logout — pending Google login)
...
### 1. Geocode iCal events for the Map screen
**Real product gap.** Club Salud (our highest-volume source) ships iCal events with rich `venueAddress` text but no coordinates. The Map screen filters by lat/lng, so most Club Salud events are invisible there. Two paths:

- **Nominatim** (free, OSM-backed). Rate limit: 1 req/sec. Use only on unique addresses (cache by address hash). Fits ~5000 unique addresses/year well within free tier.
- **LLM enrichment for iCal** — same pattern as `enrichEventsBatch` for HTML, but prompt the LLM with just the address text. LLM lat/lng can hallucinate; quality risk.

Recommend Nominatim. ~2-3 hours. Add a `geocoded_addresses` cache table or reuse description-hash style cache.

### 2. Apply migration 0005 in Manus (with pre-deploy duplicate check)
The `0005_nosy_zarda.sql` migration changes `canonicalKey`'s index from non-unique to unique AND adds `venueDateKey` with a unique index. **Run the duplicate check at the top of the SQL file first** — if existing rows have duplicate canonicalKeys, the UNIQUE creation fails mid-migration and leaves a half-applied state.

### 3. Map screen lat/lng wiring (after #1, verification)
Once geocoding lands, open the Map tab and confirm pins are dense across Tokyo. Sparse pins mean the geocoder isn't covering the venues we care about; iterate on caching or fallback.

### 4. Sanity-check the v1.5 data quality
SQL queries to check after migration applies and a fresh scrape cycle runs:
- Counts by source (expect ~14 sources active)
- % of events with address / coords / description
- Any extraction failures in `scrape_logs`

### 5. Verify the 6h scrape cadence is firing
Check `scrape_logs.createdAt` shows ~6h gaps. The constant changed in branch but the deployed server may still be running on 1h until restarted.
...
### Auth (still deferred)
- [ ] **Google OAuth login** via `expo-auth-session`. Codebase has `protectedProcedure` defined and `TODO(auth)` markers in `server/routers.ts` for the exact endpoints to flip.
- [ ] After auth: switch `sources.add`/`toggle`/`delete` and `auth.logout` from `publicProcedure` to `protectedProcedure`. Three-line change.
- [ ] After auth: implement `preferences.get` and `preferences.upsert` tRPC procedures. Helpers + mass-assignment guard already in `server/db.ts`.
...
### v1.5 scraping pipeline (this branch — pending migration)
...
| Metric | Value |
|--------|-------|
| Real adapters with working extraction | 1 (HTML via LLM); iCal next |
| Stub adapters | 3 (Facebook, Instagram, RSS — RSS being replaced by iCal) |
| Sources seeded | 7 (6× SalsaVida cities + LatinDanceCalendar Tokyo) |
| Sources to add when iCal lands | 7 (Club Salud × 4 + Meetup × 3) → total 14 |
| Events in production DB | Real (sanity-check pending) |
| Test files / tests | 17 / 267 passing, 1 skipped |
| Documentation files | 9 (some drift, see "Worth knowing") |
| Open security issues | 0 in main code; 3 deferred until auth |
| Schema migrations | 5/5 applied |
